...
class FeatureExtractor(object):
    ...
    def extract_features(self, df_parsed_log: pd.DataFrame,
    ...
    def extract_log_key_sequence_by_session(self, df_parsed_log: pd.DataFrame) \
            -> list:
        """
    ...
    def create_features_and_labels(self, log_key_sequences: list) -> pd.DataFrame:
        ...
        if self.verbose:
            print("\nFeatures and label extraction complete: {num_win} input "
                  ...
                  "{num_sess} session(s).".format(num_win=df_dataset.shape[0],
    ...
    def fit_transform(self, feature_dataset: pd.DataFrame) -> pd.DataFrame:
        """
        Apply a transformation to the log keys on a data set to be used for
        training.
    ...
    def transform(self, feature_dataset: pd.DataFrame) -> pd.DataFrame:
        ...
        Apply a transformation to the log keys on a dataset to be used
        for detection.
    ...
    def _transform(self, feature_dataset: pd.DataFrame) -> pd.DataFrame:
        ...
        feature_dataset["Label"] = feature_dataset["Label"].map(
